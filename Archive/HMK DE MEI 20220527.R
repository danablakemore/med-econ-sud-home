...
wb <- createWorkbook()
fn <- 'HMK DE MEI Summary Analysis 2019-2021.xlsx'
...
data <-
  ...
  mutate(across(ends_with('12mths'), ~ifelse(reporting_date<=start_date+months(10),NA,.x)),
         ...
         per_k_12mths = 12000*events_12mths / mm_12mths)
...
pmpm_trend_sheet <- createSheet(wb, 'PMPM Trend')
addPicture(paste0('pmpm_trend_client_id',client_id,'.png'),pmpm_trend_sheet)
...
data %>%
  ...
  facet_wrap(~wayspring_csl_enc_cat_tier_1, scales = 'free_y') +
  geom_line() +
  geom_vline(xintercept = ymd('2020-03-01'), width = .5, linetype = 'dashed') +
  scale_x_date('Event Month', date_breaks = '12 months', date_minor_breaks = '3 months',date_labels = '%Y') +
  scale_y_continuous('Events / 1,000 In-Scope Members') +
  geom_text(aes(x=ymd('2020-03-01'),y=.5,label = 'COVID'),size=3,hjust=0)
dev.off()

util_trend_sheet <- createSheet(wb, 'Utilization Trend')
addPicture(paste0('util_trend_client_id',client_id,'.png'),util_trend_sheet)
...
cost_util_summary <-
  ...
  ungroup() %>%
  mutate(`Amount Paid PMPM` = dollar(paid / mm,.01),
         `Events / 1,000` = comma(12000*events / mm,.1),
         `% Paid` = percent(paid / paid_total,.1),
         across(c('members','events'),comma),
         paid = dollar(paid,1),
  ) %>%
...
sud_res <-
  ...
  summarise(`# Stays` = comma(n()),
            `# Members` = comma(n_distinct(axial_member_id)),
            `Total # Days` = comma(sum(los),1),
            `Avg. LOS` = round(mean(los),.1),
            `Days / Member` = round(sum(los) / n_distinct(axial_member_id),.1),
            `% Residential Spend` = percent(sum(paid) / sum(events$paid[grepl('01',events$wayspring_csl_enc_cat_tier_1)]),.1),
            `Total Paid` = sum(paid),
            `Paid / Member` = dollar(sum(paid) / n_distinct(axial_member_id)),
            `Cost / Day` = dollar(sum(paid) / sum(los),.01)
  ) %>%
  arrange(-`Total Paid`) %>%
  ungroup() %>%
...
sud_res_sheet <- createSheet(wb, 'SUD Residential Stay Summary')
addDataFrame(bind_rows(sud_res_total,sud_res),sud_res_sheet,row.names = FALSE)
...
outpt_surg_sheet <- createSheet(wb, 'Outpatient Surgery') 
addDataFrame(rbind(outpt_surg_total,outpt_surg),outpt_surg_sheet)
...
lab_billing_providers <-
  ...
  summarise(`# Tests` = sum(n_tests),
            `% of Tests` = percent(sum(n_tests)/mean(annual_tests),accuracy = .01),
            `# Members` = n_distinct(axial_member_id),
            `Total Paid` = dollar(sum(paid),accuracy = 1),
            `% of Total Paid` = percent(sum(paid)/mean(total_paid),accuracy = .1),
            `Avg. Paid` = dollar(sum(paid)/sum(n_tests), accuracy = .01),
            `% Confirmatory` = percent(sum(n_conf)/sum(n_tests),accuracy = .1),
            `Avg. Paid (Conf.)` = dollar(sum(paid_conf) / sum(n_conf), accuracy = .01),
            `% Presumptive` = percent(sum(n_pres)/sum(n_tests),accuracy = .1),
            `Avg. Paid (Pres.)` = dollar(sum(paid_pres) / sum(n_pres), accuracy = .01),
            `Avg. # Tests / Visit` = comma(mean(n_tests_day),accuracy = .1),
            `Avg. # Conf. Tests / Visit` = comma(mean(conf),accuracy = .1),
            `% w/ Pres. Test` = percent(mean(pres_pre_conf,na.rm = TRUE), accuracy = .1),
            `% w/ Conf. Test` = percent(mean(conf_pre_pres,na.rm = TRUE), accuracy = .1)
  ) %>%
  ungroup() %>%
...
lab_billing_providers_sheet <- createSheet(wb, sheetName = 'Drug Tests by Billing Prov')
addDataFrame(as.data.frame(bind_rows(lab_billing_providers_total,lab_billing_providers)),lab_billing_providers_sheet,row.names = FALSE)
...
drug_test_trend_plot_sheet <- createSheet(wb, sheetName = 'Drug Testing Trend')
addPicture('drug_test_trend_plot.png', drug_test_trend_plot_sheet,scale = 1, startRow = 1, startColumn = 1)
...
ambulance_summary_sheet <- createSheet(wb,'Ambulance Summary')
addDataFrame(ambulance_summary,ambulance_summary_sheet,row.names = FALSE, startRow = 2)
...
addDataFrame(ambulance_detail,ambulance_summary_sheet,row.names = FALSE,startColumn = ncol(ambulance_summary)+2, startRow = 2)
...
# 4,907 , 10,698 7,829,881
ed_summary_total <-
  ed %>%
  mutate(avoid_cat = 'Total') %>%
  group_by(avoid_cat) %>%
  summarise(`# Visits` = n(),
            `# Members` = comma(n_distinct(axial_member_id)),
            `Total Paid` = dollar(sum(paid),1),
...
ed_summary_sheet <- createSheet(wb,'ED Summary')
addDataFrame(bind_rows(ed_summary_total,ed_summary),ed_summary_sheet,row.names = FALSE)
...
avoidable_ed_sheet <- createSheet(wb, 'Avoidable ED Detail')
addDataFrame(avoidable_ed,avoidable_ed_sheet,row.names = FALSE)
...
png(paste0('asp_plot_',client_id,'_',gsub('-','',Sys.Date()),'.png'),width=900,height=500)
med_pharm %>%
...
med_pharm_summary <-
  ...
  summarise(`# Visits` = n(),
            ...
            `Savings / Visit` = dollar(sum(amount_total_paid - asp_cost)/n(),1),
...
med_pharm_summary_sheet <- createSheet(wb,'Medical Pharmacy Summary')
addDataFrame(bind_rows(med_pharm_summary_total,med_pharm_summary),med_pharm_summary_sheet,row.names = FALSE)
addPicture(paste0('asp_plot_',client_id,'_',gsub('-','',Sys.Date()),'.png'),med_pharm_summary_sheet,startColumn = ncol(med_pharm_summary))
...
med_pharm_detail <-
  ...
  mutate(`Savings Opportunity` = dollar(ifelse(amount_insurance_paid<asp_cost,0,amount_insurance_paid-asp_cost)),
         across(c(starts_with('amount'),'asp_cost'),dollar),
         asp_unit_cost = dollar(asp_unit_cost,.001),
         pct_asp = percent(pct_asp,.1)) %>%
  rename(`Axial Member ID` = axial_member_id,
         `Service Date` = date,
         NPI = npi,
...
med_pharm_detail_sheet <- createSheet(wb, 'Medical Pharmacy Detail')
addDataFrame(med_pharm_detail,med_pharm_detail_sheet,row.names = FALSE,showNA = FALSE)
...
imaging_summary <-
  ...
  summarise(`# Tests` = n(),
            `% of Tests` = percent(n() / mean(total_tests),.1),
            `Total Paid` = dollar(sum(paid)),
            `Avg. Paid` = dollar(mean(paid),1),
            `% of Paid` = percent(sum(paid) / mean(total_paid),.1)) %>%
  ungroup() %>%
  left_join(mm %>% 
  ...
  mutate(`# Tests / 1,000` = comma(12000*`# Tests`/mm,.1),
         `# Tests` = comma(`# Tests`)) %>%
  select(-mm) %>%
...
imaging_summary_sheet <- createSheet(wb,'Imaging Summary')
addDataFrame(bind_rows(imaging_summary,imaging_summary_total),imaging_summary_sheet,row.names = FALSE,startRow = 2)
...
imaging_details_sheet <- createSheet(wb,'Imaging Details by POS')
addDataFrame(imaging_details,imaging_details_sheet,row.names = FALSE)

saveWorkbook(wb,paste0('MEI Analysis_',unique(cost_util$payor_desc),'_',gsub('-','',Sys.Date()),'.xlsx'))
